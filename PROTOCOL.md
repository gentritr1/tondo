...
| `joinRoom` | `{code, name, token?}` | `token` reclaims a disconnected seat |
| `addBot` | `{}` | host, lobby only, up to 4 seats |
| `removeSeat` | `{seatId}` | host, lobby only, bot seats only |
...
Unknown/invalid → `error` + fresh `state` snapshot.
